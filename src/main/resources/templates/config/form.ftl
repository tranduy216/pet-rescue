        ...
        <div class="flex justify-between items-center">
            ...
            <form method="get" action="/config" class="flex gap-2">
                ...
                <select name="wishStatus" onchange="this.form.submit()" class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500">
                    ...
                    <option value="PENDING" <#if (wishStatus)! == 'PENDING'>selected</#if>>${msg['wish_status_pending']!'Chờ Duyệt'}</option>
                    <option value="APPROVED" <#if (wishStatus)! == 'APPROVED'>selected</#if>>${msg['wish_status_approved']!'Đã Duyệt'}</option>
                    <option value="RECEIVED" <#if (wishStatus)! == 'RECEIVED'>selected</#if>>${msg['wish_status_received']!'Đã Nhận'}</option>
                    <option value="DELETED" <#if (wishStatus)! == 'DELETED'>selected</#if>>${msg['wish_status_deleted']!'Đã Xóa'}</option>
                </select>
            </form>
        </div>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-sm">
                            ...
                            <#elseif wish.status == "DELETED">
                                <span class="bg-red-100 text-red-700 text-xs px-2 py-1 rounded-full">${msg['wish_status_deleted']!'Đã Xóa'}</span>
                            <#else>
