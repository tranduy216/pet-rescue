...
<@layout.page title="${(blog??)?then('Edit', 'New')} Post - Pet Rescue">
<div class="max-w-2xl mx-auto mt-6">
    <div class="bg-white rounded-xl shadow-md p-8">
        <h1 class="text-2xl font-bold text-green-800 mb-6">${(blog??)?then('Edit', 'New')} Blog Post</h1>
        <#if error??>
        ...
        <form method="POST" action="/blog/<#if blog??>${blog.id}/edit<#else>new</#if>" class="space-y-4">
            ...
            <div class="flex space-x-3">
                ...
                <a href="/blog<#if blog??>/${blog.id}</#if>" class="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-300">Cancel</a>
            </div>
        </form>
    </div>
</div>
